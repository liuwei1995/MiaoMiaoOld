...
public class BaseDaoImpl<T> implements BaseDao<T> {
	...
	public boolean isExist(String sql, String[] selectionArgs) {
		...
		try {
			...
			MMLog.e(this.TAG, "[isExist] from DB Exception.");
			e.printStackTrace();
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	public List<T> find(String[] columns, String selection,
		...
		try {
			...
			MMLog.e(this.TAG, "[find] from DB Exception");
			e.printStackTrace();
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	public synchronized long insert(final T entity,final boolean flag) {
		...
		try {
			...
			MMLog.d(TAG, "[insert]: insert into " + this.tableName + " " + sql);
			long row = db.insert(this.tableName, null, cv);
			return row;
		} catch (Exception e) {
			MMLog.d(this.TAG, "[insert] into DB Exception.");
			e.printStackTrace();
		} finally {
			if (db != null) {
				db.close();
	...
	public void delete(int id) {
		...
		MMLog.d(TAG, "[delete]: delelte from " + this.tableName + " where "
	...
	public void delete(Integer... ids) {
		if (ids.length > 0) {
			...
			MMLog.d(TAG, "[delete]: " + getLogSql(sql, ids));
	...
	public void update(T entity) {
		...
		try {
			...
			MMLog.d(TAG, "[update]: update " + this.tableName + " set " + sql
					+ " where " + where.replace("?", String.valueOf(id)));

			String[] whereValue = { Integer.toString(id) };
			db.update(this.tableName, cv, where, whereValue);
		} catch (Exception e) {
			MMLog.d(this.TAG, "[update] DB Exception.");
			e.printStackTrace();
		} finally {
			if (db != null)
				db.close();
	...
	public List<Map<String, String>> query2MapList(String sql,
		...
		MMLog.d(TAG, "[query2MapList]: " + getLogSql(sql, selectionArgs));
		SQLiteDatabase db = null;
		Cursor cursor = null;
		...
		try {
			...
			MMLog.e(TAG, "[query2MapList] from DB exception");
			e.printStackTrace();
		} finally {
			if (cursor != null) {
				cursor.close();
	...
	public void execSql(String sql, Object[] selectionArgs) {
		SQLiteDatabase db = null;
		MMLog.d(TAG, "[execSql]: " + getLogSql(sql, selectionArgs));
		try {
			db = this.dbHelper.getWritableDatabase();
//			db.delete("", null, null);
			...
			MMLog.e(TAG, "[execSql] DB exception.");
			e.printStackTrace();
		} finally {
			if (db != null) {
				db.close();
