...
public class ViewHolder
{
	...
	public <T extends GridView> T getGridView(int viewId)
	{
		GridView view = (GridView) mViews.get(viewId);
		if (view == null)
		{
			view =  (GridView) mConvertView.findViewById(viewId);
			mViews.put(viewId, view);
		}
		return  (T) view;
	}
...
//				(ImageView) getView(viewId));2131362230
		ImageLoaderUtil.disPlay(url, (ImageView) getView(viewId));
		return this;
	}
	public ViewHolder setImageByUrlGlideUtil(int viewId, String url)
	{
//		ImageLoader.getInstance(3, Type.LIFO).loadImage(url,
//				(ImageView) getView(viewId));2131362230
		GlideUtil.disPlay(url, (ImageView) getView(viewId));
		return this;
	}
