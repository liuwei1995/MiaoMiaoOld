package com.zhaoyao.miaomiao.activity.music;
...
import com.zhaoyao.miaomiao.adapter.music.Viewpageradapter;
import com.zhaoyao.miaomiao.constant.MyConstant;
import com.zhaoyao.miaomiao.constant.Myhandler;
import com.zhaoyao.miaomiao.entity.music.Media;
import com.zhaoyao.miaomiao.list.Mylist;
import com.zhaoyao.miaomiao.lyr.LrcView;
import com.zhaoyao.miaomiao.lyr.LyricItem;
import com.zhaoyao.miaomiao.lyr.LyricParser;
import com.zhaoyao.miaomiao.pagerstay.ZoomOutPageTransformer;
import com.zhaoyao.miaomiao.service.music.MusicService;
import com.zhaoyao.miaomiao.view.music.VisualizerView;
...
public class PlayerActivity extends Activity {
	...
	private void init() {
		...
		switch (playmode) {
		case 1:
			mode.setImageResource(R.drawable.music_player_btn_mode_normal_style);
			break;
		case 2:
			mode.setImageResource(R.drawable.music_player_btn_mode_random_style);// 随机
			break;
		case 3:
			mode.setImageResource(R.drawable.music_player_btn_mode_repeat_all_style);// 循环
			break;
		case 4:
			mode.setImageResource(R.drawable.music_player_btn_mode_repeat_one_style);// 单曲
			break;
		}
		bomode = playmode;
		if (flag1 != flag) {
			flag = flag1;
			start.setImageResource(R.drawable.music_player_btn_pause_style);
		}
		if (Mylist.medias.size() != 0) {
			...
			if (islist(list_arr).get(position).isLove()) {
				loveimage
						.setImageResource(R.drawable.music_player_btn_favorite_star_style);
			}
			new myinit().start();
		}
	}
	...
	Handler mhandler = new Handler() {// 处理线程
		public void handleMessage(Message msg) {
			...
			case Myhandler.INIT_LOVE_FLOAT:
				loveimage
						.setImageResource(R.drawable.music_player_btn_favorite_nostar_style);
				break;
			}
		};
	};
	...
	public void start(View v) {// 开始暂停按钮
		if (Mylist.medias.size() != 0) {
			if (flag) {
				...
				start.setImageResource(R.drawable.music_player_btn_play_style);
			}
			flag = !flag;
		} else {
